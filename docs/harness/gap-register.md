...
| GAP-046 | Op de aprovação no protocolo (H10 critério 2) | pág. 13/H10 | ⬜ open | O runner marca `PendingPerms` e o run **yielda** em `permission_wait`, mas nenhum cliente consegue responder: o IDL (`internal/harness/protocol`) não tem op de aprovação, `agent resume` só avança a fase a partir do checkpoint, e `PendingPerms` não é lido por ninguém. O TUI renderiza `permission_wait` e diz que não pode responder — em vez de fingir uma aprovação. Descoberto pelo próprio spike, que é o resultado correto | adicionar a op (`approve`/`deny` com `run_id` + request id) ao IDL e ao manifest, ligar `PendingPerms` ao gate de permissão e reexecutar o turno; então o TUI passa a ter superfície de aprovação e o critério 2 fecha |
...
| GAP-047 | Critérios 4 e 5 do H10 não medidos | pág. 13/H10 | 🟡 partial | Critérios 1 (boundary) e o mecanismo de 3 (replay) têm regressão automatizada; o critério 2 passa em tudo menos a aprovação (GAP-046). Modo remoto TLS+token e budget de redraw sob a metodologia do ADR 009 ainda não foram exercitados pelo TUI | exercitar `DialRemote` com o mesmo modelo (é o mesmo cliente, outro endereço) e registrar baseline de redraw por 200 eventos antes de inventar qualquer threshold |
