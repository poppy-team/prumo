...
`prumo-agent docs agents verify`), the semantic impact graph and Goal preflight/postflight
(W17: `internal/documentation/impact_semantic.go`, `plan.go`), the publishing and
AI-retrieval plane (W18: `internal/docpublish`, `prumo-agent docs site verify`), the
...
adoption (W21: `internal/docintel`, `prumo-agent docs adopt`).
...
`prumo-agent docs readiness` is now green because 7 contracts genuinely carry accepted
