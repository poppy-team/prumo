...
| `project root not found` | Run `prumo-agent status --path <project>` and verify `prumo.json` |
| Validation failure | Run `prumo-agent doctor <project> --json` |
...
| Compiler failure | Run `prumo-agent framework-check`, then regenerate from canonical resources |
...
```bash
prumo-agent goal list --path ./project
prumo-agent goal state P00-G01 PLANNED --path ./project
prumo-agent goal amend P00-G01 --file amendment.json --path ./project
```
...
prumo-agent compile --target generic --path ./project
prumo-agent compile --target codex --path ./project
prumo-agent compile --target claude-code --path ./project
```
...
```bash
prumo-agent --home ./clean-home setup
prumo-agent --home ./clean-home install connector opencode
prumo-agent --home ./clean-home uninstall --connectors --purge-cache
```
...
Keep sanitized logs and command output. Do not include secrets, credentials, complete sensitive prompts, or classified repository contents. Use Git history, snapshots, and `prumo-agent doctor --json` as primary evidence.
