...
func runTui(asJSON bool, args []string) int {
	...
	if err := cmd.Run(); err != nil {
		return serviceError(asJSON, fmt.Errorf("prumo-tui: %w", err))
	}
	return exitOK
}
...
func tuiBinary() (string, error) {
	...
	if exe, err := os.Executable(); err == nil {
		sibling := filepath.Join(filepath.Dir(exe), "prumo-tui")
		if _, err := os.Stat(sibling); err == nil {
			return sibling, nil
		}
	}
	if path, err := exec.LookPath("prumo-tui"); err == nil {
		return path, nil
	}
	return "", fmt.Errorf("the terminal client is a separate binary: install `prumo-tui`, or set PRUMO_TUI_BIN (protocol %s)", protocol.CLIVersion)
}
